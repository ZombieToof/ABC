...
<head>
    ...
    <title>ABC &bull; Army Battleday Signups</title>
    ...
    <script type="text/javascript" language="javascript">
		...
		$(document).ready(function(e) {
			$('#battle-picker').change(function(e) {
               $(window).attr("location", "army_battleday_signup.php?battleid=" + $(this).val() + "&army=" + <?php echo $army_to_manage; ?>);
            });
            <?php if($abc_user->is_admin) { ?>
			$('#army-picker').change(function(e) {
                $(window).attr("location", "army_battleday_signup.php?army=" + $(this).val() + "&battleid=" + <?php echo $battle_to_manage; ?>);
            });
			<?php } ?>
					...
					<div class="bd-sign">
						<div class="large-heading middle-text"> Battle: <?php echo $batm->name; ?></div>
							...
							<table class="bd-soldier-details">
								<tr>
									<td></td>
									<td colspan="<?php echo (int)$batm->length; ?>" align=center><font size="5"><B>Battle Hours in (SBT)</B></font></td>
								</tr><tr>
								...
								/*for($i = 0; $i < count($battle_sign); $i++) {
									if($battle_sign[$i]['battle']->soldiers[0]['username'] == $user->data['username']) {
										echo '<tr class="user">
											<td>
												<input type="hidden" name="battle_signid" value="' . $i . '"/>
												<input type="hidden" name="signup_id" value="' . $battle_sign[$i]['battle']->id . '"/>' . $battle_sign[$i]['battle']->soldiers[0]['rank_name'] . '    ' . $battle_sign[$i]['battle']->soldiers[0]['username']. '
											</td>';
									} else {
										echo '<tr>
									...
									for($b = ((int)$batm->length - 1) ; $b >= 0; $b--) {
										if(($battle_sign[$i]['battle']->hours[$b] != 1) && ($battle_sign[$i]['battle']->soldiers[0]['username'] == $user->data['username'])) {
											...
											$signed_up = 1;
										} elseif(($battle_sign[$i]['battle']->hours[$b] == 1) && ($battle_sign[$i]['battle']->soldiers[0]['username'] == $user->data['username'])) {
											$signed_up = 1;
											$totalhours[$b] = $totalhours[$b] + 1;
											echo '<td bgcolor=#ffff00> <input type="checkbox" name="signuphour_' . $b .'" value="1" checked="checked" /></td>';
										} elseif($battle_sign[$i]['battle']->hours[$b] == 1) {
								...
								<?php if ($signed_up == 1) 
									...
									<tr>
										<td width="100"><?php $abc_user->output_soldier_rank(); ?></td>
										<?php for($i = 0; $i < ((int)$batm->length); $i++)
